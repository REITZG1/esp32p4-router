...
MixerNode::MixerNode(int fs, int channelCount) { this->begin(fs, channelCount); }

MixerNode::~MixerNode() { if(interpolator) { delete interpolator; } }
...
void MixerNode::setVolume(float vol, bool fade) {
	...
	if (fade == false) { volume[kCurrent] = volume[kTarget]; }
